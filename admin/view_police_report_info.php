...
include('db_con.php'); 
$limit = 6;  
$sql = "SELECT COUNT(id) FROM police_info_report_table";  
...
if (isset($_GET["police_page"]))
{
  $cPage=$_GET["police_page"];
   $one=6*$cPage-5;
    $two=$one+5;
...
?>
  ...
  <div class="row">
    <div class="col-md-8">
<?php
if (isset($_GET["police_page"])) { $page  = $_GET["police_page"]; } else { $page=1; };  
$start_from = ($page-1) * $limit;  

$sql = "SELECT * FROM police_info_report_table ORDER BY id DESC LIMIT $start_from, $limit";  
...
for ($i=1; $i<=$total_pages; $i++) {  
             $pagLink .= "<li><a href='action_index.php?police_page=".$i."'>".$i."</a></li>";  
};  
  echo $pagLink . "</ul></nav>"; 
...
$('.pagination').pagination({
    ...
    hrefTextPrefix : 'action_index.php?police_page='
    });
  });
</script> 
