	...
	if(isset($_POST['save']))
		{
				if(($_FILES['file']['type'] == 'image/gif') || ($_FILES['file']['type']=='image/jpeg') ||($_FILES['file']['type']=='image/png') ||($_FILES['file']['type']=='image/pjpeg') && ($_FILES['file']['size']<200000))
				{
					if($_FILES['file']['error']>0)
					{
						...
						header("location:../index.php?action=report_to_police");
					}
					...
					else if(move_uploaded_file($_FILES['file']['tmp_name'],'report_photo/'.$_FILES['file']['name']))
					{
							...
							$sql2="SELECT * FROM police_info_table WHERE upazillaName='".$_SESSION['upazillaName']."'";
							$result=mysql_query($sql2);
							$data=mysql_fetch_array($result);
							$mobileno = $data['mobileNo'];
							$police_title = $data['police_title'];
							...
							$sender = 'Emr.Service';
								...
								$statusValue=1;
						    $sql="INSERT INTO police_info_report_table(action_title,divisionName,districtName,upazillaName,placeName,reporterMobileNo,reportDescription,image,police_info_title,police_mobileNo,date_time,status,UserInfo)
			VALUES('$_POST[action_title]','$_POST[divisionName]','$_POST[districtName]','$_POST[upazillaName]','$_POST[placeName]','$_POST[mobileNo]','$_POST[reportDescription]','{$part}','$police_title','$mobileno','$dates','$statusValue','$_POST[UserInfo]')";
				...
				else
					{
							...
							$statusValue=1;
							$no_photo="NoPhoto";
					   		$sqls="INSERT INTO police_info_report_table(action_title,divisionName,districtName,upazillaName,placeName,reporterMobileNo,reportDescription,image,police_info_title,police_mobileNo,date_time,status,UserInfo)
			VALUES('$_POST[action_title]','$_POST[divisionName]','$_POST[districtName]','$_POST[upazillaName]','$_POST[placeName]','$_POST[mobileNo]','$_POST[reportDescription]','{$no_photo}','$police_title','$mobileno','$dates',$statusValue,'$_POST[UserInfo]')";
